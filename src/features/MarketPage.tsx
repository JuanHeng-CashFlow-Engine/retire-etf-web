...
export function MarketPage({ data, onNavigate }: { data: RetirementOverview; onNavigate: (page: string) => void }) {
  ...
  return <section className="jh-feature-page"><FeatureHeader title="市場大跌追蹤" subtitle="設定資產價格與配息下降情境，估算退休資產及現金流承受力。" />
    <div className="jh-feature-grid">
      ...
      <FeaturePanel number={5} title="高風險部位辨識" subtitle="依目前曝險市值與集中度排序"><div className="jh-list">{riskRows.map((item) => <div key={item.name}><span>{item.name}</span><b>{money.format(item.value)}</b></div>)}{!riskRows.length && <Empty>尚未輸入有價格的股票或 ETF。</Empty>}</div><p className="jh-muted">最大單一資產占總資產 {concentration.topPct.toFixed(1)}%。曝險部位只是情境假設，不是個別商品的風險評等。</p>{reviewAssets.length > 0 && <p className="jh-muted">另有 {reviewAssets.length} 筆基金、債券、保險或其他資產須逐筆核對後，才可加入壓力情境。</p>}</FeaturePanel>
