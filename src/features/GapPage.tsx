...
export function GapPage({ data, identity, reload, onNavigate }: { data: RetirementOverview; identity: ClaimsIdentity; reload: () => Promise<void>; onNavigate: (page: string) => void }) {
  ...
  const expenseKnown = data.monthlyExpense > 0
  const deficits = expenseKnown ? projection.months.filter((item) => item.gap < 0) : []
  ...
  return <section className="jh-feature-page">
    ...
    <div className="jh-feature-grid">
      ...
      <FeaturePanel number={2} title="缺口月份標示" subtitle="點選月份檢查來源"><div className="jh-month-grid">{projection.months.map((item) => <button key={item.key} className={`${item.level} ${selectedKey === item.key ? 'active' : ''}`} onClick={() => setSelectedKey(item.key)}><span>{item.label}</span><strong>{!expenseKnown ? '？' : item.gap >= 0 ? '✓' : money.format(item.gap)}</strong></button>)}</div><p className="jh-muted">{month.label}：預估收入 {money.format(month.total)}，生活費 {money.format(data.monthlyExpense)}，差額 {expenseKnown ? money.format(month.gap) : '待設定'}。</p><div className="jh-list">{month.events.map((event) => <div key={event.id}><span>{event.name} · {event.statusLabel}</span><b>{money.format(event.amount)}</b></div>)}{!month.events.length && <Empty>目前沒有已知的入帳來源。</Empty>}</div></FeaturePanel>
      <FeaturePanel number={3} title="缺口金額大小" subtitle="辨識最需要準備的月份"><SummaryStats items={[{ label: '最大單月缺口', value: !expenseKnown ? '待設定' : worst ? money.format(-worst.gap) : '無', tone: worst ? 'jh-red' : '' }, { label: '發生月份', value: worst?.label ?? '—' }, { label: '十二個月缺口合計', value: expenseKnown ? money.format(totalDeficit) : '待設定', tone: totalDeficit ? 'jh-red' : '' }]} />{expenseKnown && <MiniBars values={projection.months.map((item) => item.gap)} labels={projection.months.map((item) => `${item.month}月`)} negative />}</FeaturePanel>
      <FeaturePanel number={4} title="缺口原因分析" subtitle="按已保存資料辨識可核對項目"><div className="jh-list"><div><span>低於生活費月份</span><b>{expenseKnown ? deficits.length : '待設定'}／12 個月</b></div><div><span>僅有預估的配息筆數</span><b>{estimatedCount} 筆</b></div><div><span>已設定固定收入</span><b>{data.fixedIncomes.length} 筆</b></div><div><span>每月生活費</span><b>{money.format(data.monthlyExpense)}</b></div></div><p className="jh-muted">不足可能來自入帳時間差、收入未設定或支出高於收入；本頁不把時間差判定成配息下修。</p></FeaturePanel>
      <FeaturePanel number={5} title="補位來源建議" subtitle="在缺口前核對可動用資金"><div className="jh-advice">{!expenseKnown ? '請先設定每月生活費，才能估算缺口與補位金額。' : worst ? `先為 ${worst.label} 約 ${money.format(-worst.gap)} 的缺口準備現金，再核對固定收入起領時間與配息公告。` : '未來十二個月的已知收入已覆蓋目前生活費；仍需定期核對預估金額。'}</div><button className="jh-inline-link" onClick={() => onNavigate('income')}>編輯勞保、勞退與其他固定收入 →</button><button className="jh-inline-link" onClick={() => onNavigate('assets')}>核對現金資產 →</button></FeaturePanel>
