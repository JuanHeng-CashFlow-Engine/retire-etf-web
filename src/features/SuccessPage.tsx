...
export function SuccessPage({ data, onNavigate }: { data: RetirementOverview; onNavigate: (page: string) => void }) {
  ...
  const latestSaved = data.snapshotsV3[0]?.payload.metrics.success_probability ?? data.monthlyReports[0]?.monte_carlo_success_pct ?? data.gps?.success_probability ?? null
  const saved = [...data.snapshotsV3.map((item) => ({ key: item.as_of, value: item.payload.metrics.success_probability })), ...data.monthlyReports.map((item) => ({ key: item.report_month.slice(0, 7), value: item.monte_carlo_success_pct })), ...data.gpsHistory.map((item) => ({ key: item.snapshot_date.slice(0, 10), value: item.success_probability }))].filter((item) => item.value != null && Number.isFinite(Number(item.value))).map((item) => ({ ...item, value: Number(item.value) })).sort((a, b) => a.key.localeCompare(b.key))
  ...
  return <section className="jh-feature-page"><FeatureHeader title="退休成功率變化" subtitle="查看保存的歷史結果，並用目前設定比較可調整的退休情境。" />
    <div className="jh-feature-grid">
      ...
      <FeaturePanel number={5} title="安全區間燈號" subtitle="按目前模擬成功率分類"><div className="jh-big-stat"><strong className={success == null ? '' : success >= 85 ? 'jh-green' : success >= 70 ? '' : 'jh-red'}>{success == null ? '待設定' : success >= 85 ? '安全' : success >= 70 ? '注意' : '警戒'}</strong><small>安全 ≥85%；注意 70–85%；警戒 &lt;70%。這只是情境標示。</small></div><p className="jh-muted">模擬依固定報酬與波動假設計算，不能保證實際退休結果。</p></FeaturePanel>
