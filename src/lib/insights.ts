...
export function assetConcentration(data: RetirementOverview) {
  const rows = [
    ...data.holdings.map((item) => ({ name: item.name, value: holdingMarketValue(item) })),
    ...data.assets.map((item) => ({ name: item.asset_name, value: Number(item.current_value) || 0 })),
...
export function buildAlerts(data: RetirementOverview, success: number | null): InsightAlert[] {
  ...
  if (concentration.top && concentration.topPct >= 40) alerts.push({
  ...
  for (const quote of data.quotes) {
    ...
    alerts.push({ type: 'dividend_drop', severity: 'danger', title: `${quote.ticker} 股利下降超過 20%`,
      message: `市場主檔顯示股利變動 ${change.toFixed(1)}%。${annualLoss == null ? '' : `按目前持有量估算，年收入可能減少約 ${money.format(annualLoss)}。`}`,
...
export function buildRecommendations(data: RetirementOverview, success: number | null): string[] {
  ...
  if (assetConcentration(data).topPct >= 40) recommendations.push('檢查單一資產集中度，避免退休資產過度依賴單一標的。')
  if (!recommendations.length) recommendations.push('維持定期核對資產、生活費與配息資料。')
  return recommendations
}

export function healthScore(data: RetirementOverview, success: number | null): number | null {
  if (success == null || !data.goal || data.monthlyExpense <= 0) return null
  const cap = (value: number) => Math.min(100, Math.max(0, value))
  const weight = assetConcentration(data).topPct
  const concentrationScore = weight >= 60 ? 30 : weight >= 40 ? 55 : weight >= 25 ? 75 : 100
  const dividendScore = Math.max(0, 100 - data.quotes.reduce((penalty, quote) => {
    const status = quote.dividend_status?.toLowerCase()
    return penalty + (status === 'danger' ? 25 : status === 'warning' ? 12 : status && status !== 'normal' ? 4 : 0)
  }, 0))
  return Math.round((cap(data.metrics.progressPct) * .2 + cap(success) * .35 + cap(data.metrics.coveragePct) * .2 + concentrationScore * .15 + dividendScore * .1) * 10) / 10
}

export function simulateOverview(data: RetirementOverview, overrides: { monthlyContribution?: number; monthlyExpense?: number; annualReturn?: number } = {}, simulations = 800): RetirementPlanResult | null {
