...
原版資料庫還沒有獨立的固定收入表。管理員須執行 `database/20260918_fixed_incomes.sql`，再確認是否已有 V3 `retirement_snapshots_v3`；若尚未建立，執行 `database/20260919_retirement_snapshots_v3.sql`。兩者均限制會員只能讀寫自己的資料，V3 快照只允許新增與讀取。未執行遷移時，相應功能會顯示設定提示。試用與 Pro 的保存按鈕由前端會員狀態控制；若付費權益需防止直接呼叫 API，正式環境應另設受控後端權限檢查。
